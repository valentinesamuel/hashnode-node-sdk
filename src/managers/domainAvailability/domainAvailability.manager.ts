...
import { CHECK_CUSTOM_DOMAIN_AVAILABILITY_QUERY, CHECK_SUBDOMAIN_AVAILABILITY_QUERY } from "./domainAvailability.queries";
...
export class DomainAvailabilityManager extends BaseManager {
    ...
    async checkSubdomainAvailability(subdomain: string) {
        const res = await this.makeRequest<{ checkSubdomainAvailability: boolean }>('checkSubdomainAvailability', CHECK_SUBDOMAIN_AVAILABILITY_QUERY, { subdomain });
     ...
     */
    async checkCustomDomainAvailability(input: CheckCustomDomainAvailabilityInput) {
        const res = await this.makeRequest<{ checkCustomDomainAvailability: boolean }>('checkCustomDomainAvailability', CHECK_CUSTOM_DOMAIN_AVAILABILITY_QUERY, {
            input
        });
